...
def load_image(image_path, size):
    """Load and preprocess image with existence check"""
...
def dynamic_cfd(V_fd):
    ...
    return np.clip(C_fd, 1e-17, 10e-13)
...
def process_and_plot(image_path, image_size, I_max, scenario_label, pdf_name, exposure_type):
    """Main processing pipeline with improved exposure simulation"""
    img_arr = load_image(image_path, image_size)
    
    # Realistic exposure simulation
    if exposure_type == "dim":
        ref_img = (img_arr * 0.5).clip(0, 1) * 255  # Conservative underexposure
    elif exposure_type == "bright":
        # Sigmoidal response for overexposure simulation
        ref_img = (img_arr * 2).clip(0, 1) * 255
    else:
        ref_img = img_arr * 255

    I_photo = img_arr * I_max  # Photocurrent simulation

    # Static capacitance processing
    V_fd_static = np.clip(V_reset - (I_photo * T_int) / C0, 0, V_reset)
    #img_static = (V_fd_static / V_reset  * 255).astype(np.uint8)
    SNR_static = 20 * np.log10(np.maximum(V_fd_static, noise_std) / noise_std)
    
    # Dynamic capacitance processing
    V_fd_dynamic = dynamic_pixel_output(I_photo, T_int)
    #img_dynamic = (V_fd_dynamic / V_reset * 255).astype(np.uint8)
    ...
    # CG_static = 1e6 * V_reset / (C0 / q_e)
    # CG_dynamic = 1e6 * V_reset / (C_fd_dynamic / q_e)
    ...
    def save_vfd_cfd_to_csv(V_fd_array, C_fd_array, filename="vfd_cfd_data.csv"):
        """
        Save V_fd and corresponding C_fd values to a CSV file.
        Each row contains: Pixel_Index, V_fd (V), C_fd (F)
        """
    ...
    # Call this after computing V_fd_dynamic and C_fd_dynamic
    save_vfd_cfd_to_csv(V_fd_dynamic, C_fd_dynamic, "python_vfd_cfd_dim_v1.csv")
    ...
    titles = ['Under-exposed', 'Fixed $C_{\mathrm{FD}}$', 'ReRAM at FD', 'ΔCG (μV/e⁻)', 'ΔSNR (dB)']
...
# Execute both scenarios with image display
process_and_plot(image_path, image_size, I_max_dim, "Dim Light", "python_dim_FROZEN_redo.png", "dim")
